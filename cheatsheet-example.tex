...
% Font size. (Can go a bit bigger with `\footnotesize` or `\scriptsize`.)
\tiny
...
% This is where the Rmarkdown stuff comes in.
\section{TIPT}
...
For spherical symmetric potential: \begin{align*}
...
Let \(V_0 = q_1q_2/4\pi \epsilon_0\), \(R=\infty\): \begin{align*}
...
Domain: \((-1,1)\) \textbackslash{} Even, Odd, Even, Odd \ldots{}
\begin{align*}
    P_0(x) &= 1 \\
    P_1(x) &= x \\
    P_2(x) &= \frac12 (3x^2-1) \\
...
Solution to Radial Shrodinger Equation: \begin{align*}
...
\end{align*} When \(kr >> 1\) \begin{align*}
...
Domain: \((-\infty,\infty)\) \textbackslash{} Even, Odd, Even, Odd
\ldots{} \begin{align*}
    H_0(x) &= 1 \\
    H_1(x) &= 2x \\
    H_2(x) &= 4x^2-2 \\
...
For a Linear Operator \(\hat{D}_x\) \begin{align*}
